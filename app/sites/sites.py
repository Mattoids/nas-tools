...
class Sites:
    ...
    def init_config(self):
        ...
        for site in self._sites:
            ...
            if site_uses:
                ...
                statistic_enable = True if "T" in site_uses and (site_rssurl or site_signurl) and (
                            site_cookie or site_apikey) else False
    ...
    def test_connection(self, site_id):
        ...
        if site_info.get("chrome") and chrome.get_status():
            # 计时
            start_time = datetime.now()
            if not chrome.visit(url=site_url, ua=ua, cookie=site_cookie, proxy=site_info.get("proxy"), apikey=apikey):
