...
class SiteUserInfo(object):
    ...
    def build(self, url, site_id, site_name,
              site_cookie=None, ua=None, emulate=None, proxy=False, apikey=None):
        ...
        if 'm-team' in site_domain_url:
            return MTeamTorrentUserInfo(site_name, site_domain_url, site_cookie, "", session=session, ua=ua,
                                        apikey=apikey, emulate=emulate, proxy=proxy)

        # 检测环境，有浏览器内核的优先使用仿真签到
        chrome = ChromeHelper()
        if emulate and chrome.get_status():
            if not chrome.visit(url=url, ua=ua, cookie=site_cookie, proxy=proxy, apikey=apikey):
        ...
        if not site_schema:
            ...
            return None
        return site_schema(site_name, url, site_cookie, html_text, session=session, ua=ua, emulate=emulate, proxy=proxy, apikey=apikey)
