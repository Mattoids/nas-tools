...
Docker：https://hub.docker.com/repository/docker/mattoids/nas-tools
...
### 1、站点资源不显示部分站点、索引器无部分站点、刷流无免费选项
* 前往`第三方插件`页面安装`同步索引规则`插件
* 将上面四个开关全部打开然后确定
* 同步周期建议`0 0 * * *`每天零点同步一次（最新版本新增站点的时候会通知插件进行同步，所以不需要太过频繁的同步）
* 如果操作完以后还是出现上述问题，清点击`同步历史`查看对应站点。若显示`站点刷流规则不存在，请联系作者进行适配!`则按照说明联系我进行站点适配。

### 2、Jackett、Prowlarr 插件安装配置
* 因为插件无法进行测试，所以配置完以后，只要图标右下角有绿色小点就说明配置成功
* 配置成功后，站点在`内建索引器`里面就可以看到这两个插件对应的站点
* 勾选以后就可以使用插件进行搜索了
...
* 群晖套件版本，ssh到后台运行以下命令，同样修改配置文件路径以及源目录、目的目录参数。
   ```
   export NASTOOL_CONFIG=/var/packages/mattoids/target/config/config.yaml
   /var/packages/py3k/target/usr/local/bin/python3 /var/packages/mattoids/target/app/filetransfer.py -m link -s /from/path -d /to/path
   ```
