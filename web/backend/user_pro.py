...
class UserPro(UserMixin):
    ...
    def __init__(self, user=None):
        ...
        custom_brush = json.loads(json.dumps(SystemConfig().get("plugin.CustomBrush") or {}))
    ...
    def get(self, user_id):
        ...
        for user in self.dbhelper.get_users():
            if not user:
                continue
            if user.ID == user_id:
                return UserPro({"id": user.ID, "admin": 0, "name": user.NAME, "password": user.PASSWORD, "pris": user.PRIS, "level": 2, "search": 1})
        return None
